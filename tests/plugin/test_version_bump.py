...
def test_audit_reports_all_surfaces_and_tag() -> None:
    ...
    assert "v0.2.2" in result.stdout, "--audit must report the expected git tag (v<version>)"
...
def test_check_with_matching_expect_passes() -> None:
    result = _run("--check", "--expect", "0.2.2")
    assert result.returncode == 0, "--expect 0.2.2 should pass" + _diag(result)


def test_check_with_mismatched_expect_fails() -> None:
    result = _run("--check", "--expect", "9.9.9")
    assert result.returncode != 0, "--expect 9.9.9 must fail against a 0.2.2 repo"
