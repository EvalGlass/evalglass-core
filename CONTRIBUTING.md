...
Pre-release (`v0.2.1`, pre-alpha). The plugin release gates — version alignment, strict manifest validation, the honesty audit, and the deletion-invariant — are enforced by the `tests/plugin/` suite and checked before tagging. Tagging and any PyPI / marketplace publishing are deliberate maintainer steps, not automated yet.
